...
namespace Connect4
{
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            while (!isGameOver)
            {
                ...
                else
                {
                    ...
                    {
                        int column, row;
                        if (int.TryParse(key2.KeyChar.ToString(), out column))
                        {
                            column -= 1;

                            if (connect4.IsValidMove(column))
                            {
                                row = connect4.GetNextOpenRow(column);
                                connect4.board[row, column] = currentPlayer;
        ...
        static string GetPlayerName(int currentPlayer, string player1Name, string player2Name)
        {
